...
pub fn exo(
    ...
    let path = git::init_playground(&parent_path, "rebase_i", lang);

    // Create the Git mess :-)
    // Create feature branch with 7 messy commits
...
mod tests {
    ...
    #[test]
    fn feature_branch_has_seven_commits() {
        use git2::Repository;
        use std::path::Path;
        ...
        // Stop at the commit before feature branch creation (main branch tip)
        ...
        walk.hide(main_commit.id()).unwrap();
        for _ in walk {
            count += 1;
        }
        assert_eq!(count, 7, "feature branch should have 7 commits");
    }
}
